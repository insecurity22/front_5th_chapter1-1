...
일단 배포하는데서 삽질을 정말 많이 했는데요 🥲

1. Github Pages 설정은 간단해 배포하는데 어려움은 없었지만..
2. 배포 이후 404 경로 문제가 발생했고 vite 설정 파일에 baseURL 적용 & 홈, 프로필 등 관련 탭 이동하는 주소에 baseURL을 추가하여 적용해주니 문제가 해결되었습니다.
3. 새로고침 시에도 마찬가지로 404 오류가 뜨는 것을 확인할 수 있었는데요. 이것이 SPA 문제로 인한 문제라는 것을 알았고, 이후 저는 브라우저 라우터와 해시 라우터의 의미를 명확하게 알았습니다. 이 두 라우터를 이용해 SPA 문제를 해결했습니다.

   3-1) 404.html에서 index.html로 리다이렉트하여 pathname을 해시 라우터로 전달
   3-2) index.html에서는 해시 라우터를 받아 브라우저 라우터로 변경
